...
$(window).on("load", function(){
    ...
    $("#submitter").click(function () {
		let myVar = $(this).data('name');
        logData("pageClosed - " + myVar + "1", "pageClosed - " + myVar + "1");
	...
	$("#submitter2").click(function () {
		let myVar = $(this).data('name');
		logData("pageClosed - " + myVar + "2", "pageClosed - " + myVar + "2");
...
function recordRemark(source) {
	...
	if (instance.hunkSide == 1) {
		...
		if (realLineNumber in reviewRemarksRight[instance.hunkId]) {
			if (msg == "") {
				// DELETE COMMENT
				logData(`deletedComment - ${source}`, `${instance.hunkId}${instance.hunkSide}-${realLineNumber}`);
			...
			if (msg == "") {
				// CANCEL COMMENT
				logData(`cancelComment - ${source}`,
					`${instance.hunkId}${instance.hunkSide}-${realLineNumber}`)
			} else {
				// ADD COMMENT
				logData(`addComment - ${source}`,
					`${instance.hunkId}${instance.hunkSide}-${realLineNumber}-${msg}`)
